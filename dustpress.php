...
final class DustPress
{
  ...
  protected function __construct()
  {
    ...
    spl_autoload_register(function ($class) {
      ...
      if ($class == "DustPressHelper") {
        ...
        $class = $this->camelcaseToDashed($class, "-");
      }
  ...
  public function createInstance()
  {
    ...
    $template = $this->getTemplateFilename($debugs);
    ...
    if (class_exists($template)) {
      ...
      $template_override = $this->model->getTemplate();

      $partial = $template_override ? $template_override : strtolower($this->camelcaseToDashed($template));
  ...
  private function getTemplateFilename(&$debugs = array())
  {
    global $post;
  ...
  public function render($args = array())
  {
    ...
    if (!$data) {
      ...
      $this->model->data['WP'] = $this->populateDataCollection();
    }
...
<?php
   ...
   *  @return  $data (json)
   */
  public function setDebuggerData($key, $data)
  {
    if (empty($key)) {
   ...
   *  @return  (string)
   */
  private function camelcaseToDashed($string, $char = "-")
  {
   ...
   *  @return  (boolean)
   */
  private function wantAutoload()
  {
    $conditions = [
      function () {
        return !$this->isDustpressAjax();
      },
      function () {
        return !is_admin();
      },
   ...
   *  @return  (boolean)
   */
  private function isDustpressAjax()
  {
   ...
   *  @return  N/A
   */
  public function createAjaxInstance()
  {
    global $post;
    ...
    if (!$model) {
      ...
      $model = $this->getTemplateFilename();
    ...
    if (isset($data["render"]) && $data["render"] === "true") {
      $partial = strtolower($this->camelcaseToDashed($model));
    }
    ...
    if (class_exists($model)) {
      ...
      if (empty($partial)) {
        ...
        $template_override = $instance->getTemplate();

        $partial = $template_override ? $template_override : strtolower($this->camelcaseToDashed($partial));
  ...
  {
    $filename = $this->getPrerenderFile($partial);
   ...
   *  @return  $file (string)
   */
  public function getPrerenderFile($partial)
  {
    $templatefile = $partial . '.dust';
   ...
   *  @return  N/A
   */
  public function enqueueHelpers($helpers)
  {
    if (is_array($helpers)) {
